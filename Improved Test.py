...
def clean_df(data): 
    ...
    freq_df = freq_df[freq_df.columns[freq_df.sum()> 0]] 
    #freq_df.index = idx 
    freq_df = freq_df[~freq_df.index.duplicated(keep='first')] 
    
    ##notice that last one is 0 hence delete it!
    #freq_df = freq_df.iloc[:, :-1] 
    #freq_df = freq_df.reindex(freq_df.sum().sort_values().index, axis=1)
    #freq_df = freq_df.columns[::-freq_df.shape[0]]
    freq_df = freq_df.assign(sum=freq_df.sum(axis=1)).sort_values(by='sum', ascending=False).iloc[:, :-1]
